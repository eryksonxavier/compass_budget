...
1. **Levantamento dos recursos on-premises e planejamento da migração:**  
   - Inventariar servidores, banco de dados, armazenamento e configurações atuais.
...
- **Amazon RDS:** Banco de dados MySQL gerenciado com alta disponibilidade (Multi-AZ não é necessário neste estágio, mas a instância é gerenciada e pode ser replicada futuramente).
...
- **Proteção de Dados:**  
  - Criptografia dos dados em repouso (EBS, RDS) usando AWS KMS e criptografia em trânsito com TLS/SSL.
...
5. **Configuração do Autoescalonamento:**  
   - Habilitar Auto Scaling para os nós do EKS (via Auto Scaling Groups) e usar o Horizontal Pod Autoscaler (HPA) para ajustar o número de pods conforme a carga.
...
![Infra Moderna Doc](https://github.com/user-attachments/assets/dd0ae344-fbba-47d8-a0d7-07f25f3ee530)
...
- **VPC (10.0.0.0/16)**
  ...
  - **Subnets Privadas (em AZ1 e AZ2):**
    - **Amazon EKS Cluster:**  
      ...
      - **IRSA:** Aplicado para garantir que todos os pods tenham funções IAM restritas.
    - **Amazon RDS (Multi-AZ):**  
  ...
  - **Outros Serviços Globais (não vinculados a VPC):**
    ...
    - **AWS Secrets Manager:** Armazena segredos de forma segura.
